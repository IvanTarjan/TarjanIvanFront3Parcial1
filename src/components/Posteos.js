...
import dataPosteos from './data.json'
...
export default function Posteos({handleClickLikesTotales}) {


  return (
    <div className='container'>
      {dataPosteos.map(postData=>(
        <Post key={postData.id} postData={postData} handleClickLikesTotales={handleClickLikesTotales}/>
      ))}
    </div>
  )
}
